...
```{r}
print(ggpairs(train_tmp, title="Pairwise scatter plot of variables"))
cor_matrix <- cor(train_tmp[sapply(train_tmp, is.numeric)])
print(cor_matrix)
...
```{r}
# original
index <- createDataPartition(train_tmp$Y, p = 0.8, list = FALSE)
train_df <- train_tmp[index, ]
val_df <- train_tmp[-index, ]
head(train_df)
# cross-validation (10)
...
```{r}
test_df <- test
pca_result_test <- prcomp(test_df, center = TRUE, scale. = TRUE)
summary(pca_result_test)
fviz_eig(pca_result_test, addlabels = TRUE)
fviz_pca_var(pca_result_test, col.var = "black")
cum_var <- cumsum(pca_result_test$sdev^2) / sum(pca_result_test$sdev^2)
num_components <- which(cum_var >= 0.95)[1]
test_df <- pca_result_test$x[, 1: num_components]
test_df <- as.data.frame(test_df)

pred_radical <- predict(svm_model_radical, newdata=test_df)
y.guesses <- c(pred_radical)
y.guesses
table(y.guesses)
nrow(test_df)
```
