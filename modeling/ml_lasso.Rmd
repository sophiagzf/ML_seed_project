...
```{r}
library(glmnet)
x <- as.matrix(train[, -which(names(train) == "Y")])
y <- train$Y
...
coefficients <- as.matrix(coef(model_lasso, s = "lambda.min"))
coefficients_df <- as.data.frame(coefficients)
coefficients_df <- coefficients_df[abs(coefficients_df[,1]) > 1e-4, , drop = FALSE]
names(coefficients_df)[1] <- "Coefficient"
print(coefficients_df)
plot(cv_fit)
selected_features <- rownames(coefficients_df)[-1] 
reduced_dataset <- train[, c(selected_features, "Y")]
str(reduced_dataset)
reduced_dataset$Y <- as.factor(reduced_dataset$Y)
```
...
```{r}
test_df <- test
pred <- predict(logistic_model, newdata=test_df, type = "response")
pred_class <- ifelse(pred > 0.5, 1, 0)
pred_class <- as.factor(pred_class)
y.guesses <- c(pred_class)
y.guesses
table(y.guesses)
nrow(test_df)

test.acc <- accuracy_blr
